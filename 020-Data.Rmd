...
sds_table(Galton, show_n = 10,
          caption="Figure 2.1: The `Galton` data frame containing Galton's measurements of 898 adult children.") %>%
  knitr::asis_output()
```
...
Historically, when data was shared by printing it and when calculations were tedious, data would often be presented as *tabulations*. For instance, one of the very early (@punnett-1905, p. 93) investigations of cross-linkage in genetics examined `r sum(c(480,144,132,43))` sweet pea plants, recording the color of the flower and whether the pollen was round or elongated. 

```{r punnett-93, echo=FALSE, fig.cap="Figure 2.2: Genetics data from 1905"}
...
Punnett <- Punnett[expand, c(1,2)] %>% 
  mutate(ID = paste0("SP", row_number() * 10)) %>%
  shuffle() %>%
  select(ID, flower_color, pollen_shape)
 # save(Punnett, file =  "data/Punnett.rda")
sds_table(Punnett, caption = "Figure 2.3: Punnett's data in a contemporary format")
```

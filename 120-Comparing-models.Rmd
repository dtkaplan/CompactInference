...
We'll do the simulation using R software and the `mosaic` package. I  provide  the code so that anyone who  wants to  carry out their own  simulation can do so.  We'll do 1000 trials.  `r set.seed(101)`
...
```{r allP, echo=FALSE, cache = TRUE}
All  <- data_frame(result = 0, df = rep(0, 100))
one_p <- function(n  = 11, p = 5) {
  res <- do(1000) * (
...
for  (p in 1:10) {
  foo <- data.frame(result = one_p(11, p), df=p)
  All <- rbind(All, foo)
}
onAver  <- df_stats(result ~ df,  data = All, mean=mean, sd = sd)
```
...
```{r randomR, echo=FALSE, fig.cap="(ref:RandomR-cap)"}
blue  <- data.frame(mean = c(0, 0.65, 1), df=c(0,3,10))
Labels <- data.frame(x = c(1.5, 7), y  = c(0.4, 0.9), 
                     label=c("Model segment", "Residual segment"))
gf_errorbar(mean + mean ~ df,  data = onAver) %>%
  gf_line(mean ~ df, linetype = 3, data = onAver, inherit=FALSE) %>%
  gf_jitter(result ~ df, data = All, 
            inherit = FALSE,  width = 0.1, alpha = 0.05, size = 0.5) %>%
  gf_point(mean  ~ df, data = blue[2,], size=2, color="blue") %>%
  gf_line(mean ~ df, data = blue, color= "blue") %>%
  gf_text(y ~ x, label = ~ label, data  = Labels, color  = "blue", inherit = FALSE, angle = ~ c(55, 18)) %>%
  gf_labs(y = "R-squared", x = "Degrees of flexibility")
...
```{r F-vs-df,  echo=FALSE, fig.cap="(ref:F-vs-df-cap)"}
Ffromtrials <- All %>%
  filter(df != 0, df!= 10) %>%
  mutate(F = (result/df)  / ((1-result)/(10- df))) %>%
  filter(F < 10)
meanF <- onAver %>%
  filter(df != 0, df!= 10) %>%
  mutate(F = (mean/df)  / ((1-mean)/(10- df)))
gf_errorbar(F + F ~ df,  data = meanF) %>%
  gf_line(F ~ df, linetype = 3, data = meanF, inherit=FALSE) %>%
  gf_jitter(F ~ df, data = Ffromtrials, 
            inherit = FALSE,  width = 0.1, alpha = 0.05, size = 0.5) %>%
  gf_labs(y  = "F statistic",  x = "Degrees of freedom")
```
...
```{r two-models, echo=FALSE, fig.cap="(ref:two-models-cap)"}
blue  <- data.frame(mean = c(0, 0.65, .85, 1), df=c(0,3,6, 10))
Labels <- data.frame(x = c(1.5, 4.5, 8), y  = c(0.4, 0.80, 0.97), 
                     label=c("Model one segment", "Model two segment", "Residual segment"))
gf_errorbar(mean + mean ~ df,  data = onAver) %>%
  gf_line(mean ~ df, linetype = 3, data = onAver, inherit=FALSE) %>%
  gf_jitter(result ~ df, data = All, 
            inherit = FALSE,  width = 0.1, alpha = 0.05, size = 0.5) %>%
  gf_point(mean  ~ df, data = blue[2:3,], size=2, color="blue") %>%
  gf_line(mean ~ df, data = blue, color= "blue") %>%
  gf_text(y ~ x, label = ~ label, data  = Labels, color  = "blue", inherit = FALSE, angle = ~ c(55, 22, 14)) %>%
  gf_labs(y  = "R-squared", x = "Degrees  of flexibility")
